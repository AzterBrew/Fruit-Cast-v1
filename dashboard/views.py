...
def notifications(request):
    ...
    if account_id:
        ...
        crops = [crop for crop in plant_records if crop] 
...
def forecast(request):
    ...
    if map_commodity_id and map_month and map_year:
        try:
            ...
            ).exclude(municipality_id=14).select_related('municipality') 
...
def forecast_bycommodity(request):
    ...
    filter_month = request.GET.get('filter_month') or "1"  
    filter_year = request.GET.get('filter_year') or "2025" 
    selected_municipality_id = request.GET.get('municipality_id') or "14"  
